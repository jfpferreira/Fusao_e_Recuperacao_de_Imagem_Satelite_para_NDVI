...
/*
date_pair1 - Landsat Image at date 1 (pair)
date_pair2 - Landsat Image at date 2 (pair)
...
*/
var predict = function(window_pixels, k, band, roi, date_pair1, date_pair2, date_predicted) {
    ...
    var lst_img1 = date_pair1.select([band, 'latitude', 'longitude'], [band + '_LSTPair', 'latitude', 'longitude'])
        .addBands(temporalDifference(date_pair1, date_predicted), ['TempDiff'])
        .addBands(spectralDifference(modis_pair1, date_pair1, band), ['SpectralDiff'])
        .addBands(modis_pair1.select([band], [band + '_ModisPair']))
        .addBands(modis_predicted.select([band], [band + '_ModisPredicted']))
        .addBands(date_predicted.select([band], [band + '_LSTPredicted']))

    // Landsat_Pair with TempDiff/ SpectralDiff/ NDVI_ModisPair/ NDVI_ModisPredicted
    var lst_img2 = date_pair2.select([band, 'latitude', 'longitude'], [band + '_LSTPair', 'latitude', 'longitude'])
        .addBands(temporalDifference(date_pair2, date_predicted), ['TempDiff'])
        .addBands(spectralDifference(modis_pair2, date_pair2, band), ['SpectralDiff'])
        .addBands(modis_pair2.select([band], [band + '_ModisPair']))
        .addBands(modis_predicted.select([band], [band + '_ModisPredicted']))
        .addBands(date_predicted.select([band], [band + '_LSTPredicted']))
    ...
    var euclideanDistance = function(img, original) {
        ...
        var ed = img.expression('sqrt(pow((lt-ltc),2) + pow((lg-lgc),2))', {
    ...
    var weight = function(lstImage) {

        var cluster_img = clustering(lstImage.unmask(0));

        var allCandidatesArrays = cluster_img.addBands(candidates('cluster', cluster_img)).addBands(candidates(band + '_LSTPair', cluster_img)).addBands(candidates(band + '_ModisPair', cluster_img))
            .addBands(candidates(band + '_ModisPredicted', cluster_img)).addBands(candidates('latitude', cluster_img))
            .addBands(candidates('longitude', cluster_img)).addBands(candidates('TempDiff', cluster_img)).addBands(candidates('SpectralDiff', cluster_img))

        var mask_clusters = (allCandidatesArrays.select('cluster').eq(allCandidatesArrays.select('cluster_candidates')))
        //.and(allCandidatesArrays.select(band+'_LSTPair').neq(ee.Image(-1)))
        ...
        var final1 = cluster_img.addBands(candidates_img).addBands(euclideanDistance(candidates_img, lstImage))
        ...
        return final.select('NormalizedC').addBands(ndviValues.select(band + 'Values')).addBands(nCandidates.select('nCandidates')).addBands(cluster_img.select('cluster'))

    }


    var image1 = weight(lst_img1);
    var image2 = weight(lst_img2);


    var totalWeight_candidates = image1.select('NormalizedC').arrayReduce(ee.Reducer.sum(), [0]).arrayGet([0])
        .add(image2.select('NormalizedC').arrayReduce(ee.Reducer.sum(), [0]).arrayGet([0]))

    var weight1 = image1.select('NormalizedC').divide(totalWeight_candidates).rename('weight')
    var weight2 = image2.select('NormalizedC').divide(totalWeight_candidates).rename('weight')
    var finalPredicted1 = (weight1.select('weight').multiply(image1.select(band + 'Values'))).arrayReduce(ee.Reducer.sum(), [0]).arrayGet([0]).rename(band)
    var finalPredicted2 = (weight2.select('weight').multiply(image2.select(band + 'Values'))).arrayReduce(ee.Reducer.sum(), [0]).arrayGet([0]).rename(band)

    var final = finalPredicted1.add(finalPredicted2).addBands(image1.select('nCandidates').add(image2.select('nCandidates')))

    var error_lst = date_predicted.select(band).subtract(final.select(band)).rename('error_LST')
    var error_modis = modis_predicted.select(band).subtract(final.select(band)).rename('error_Modis')

    return final.addBands(date_predicted.select(band).rename(band + '_LSTPredicted')).addBands(modis_predicted.select(band).rename(band + '_ModisPredicted'))
        .addBands(ee.Image.pixelLonLat()).addBands(error_lst).addBands(error_modis).addBands(image1.select(['cluster'], ['cluster1'])).addBands(image2.select(['cluster'], ['cluster2']))
...
app.createHelpers = function() {
    ...
    app.refreshMapCompare = function() {
        ...
        if (predictedDate) {
            ...
            maps[0].addLayer(predictedDate.select('NDVI').clip(roi), ndviParams, 'OriginalImage');
            maps[0].addLayer(predictedDate.select('NDVI').unmask(-2).eq(-2).updateMask(predictedDate.select('NDVI').unmask(-2).eq(-2)).clip(roi), {
                palette: '#b30000'
            }, 'masked');
        ...
        if (app.KW.applyNDVI.getValue()) {
            ...
            w.forEach(function(nameW, indexW) {
                k.forEach(function(nameK, indexK) {
                    ...
                    print('---------K=' + nameK + '------------')
                    var imageNIR = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'nir', roi,
                        date_pair1, date_pair2, date_predicted);
                    var imageRED = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'red', roi,
                        date_pair1, date_pair2, date_predicted);
                    var ndvi = imageNIR.select('nir').addBands(imageRED.select('red')).normalizedDifference(['nir', 'red']).rename('NDVI');
                    var error_LST = date_predicted.select('NDVI').subtract(ndvi.select('NDVI')).rename('error_LST')
    ...
    app.exportPredictedImage = function() {
        ...
        if (predictedDate && pairDate1 && pairDate2) {
            if (app.KW.applyNDVI.getValue()) {
                ...
                Export.image.toDrive({
                    image: ee.Image(ndvi).updateMask(image.select('NDVI').neq(0)).select('NDVI').clip(roi).visualize(ndviParams),
    ...
    app.exportMetrics = function() {
        ...
        var makePrediction = function(indexW, indexK, nameW, nameK, date_pair1, date_pair2, date_predicted) {
          ...
          // Reflectance
          var imageNIR = predict(w[indexW], k[indexK], 'nir', roi, date_pair1, date_pair2, date_predicted);
          var imageRED = predict(w[indexW], k[indexK], 'red', roi, date_pair1, date_pair2, date_predicted);          var ndvi = imageNIR.select('nir').addBands(imageRED.select('red')).normalizedDifference(['nir', 'red']).rename('NDVI');
          var error_LST = date_predicted.select('NDVI').subtract(ndvi.select('NDVI')).rename('error_LST')
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'STARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'WSTARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'RSTARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'BSTARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '1STARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '2STARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '3STARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '4STARFM2_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' + app.picker.pairDate2.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        })
...
app.createPanels = function() {
    ...
    app.intro = {
        panel: ui.Panel([
            ...
            ui.Label('This app allows you to apply STARFM using 2Pair Landsat-Modis')
        ])
    };
