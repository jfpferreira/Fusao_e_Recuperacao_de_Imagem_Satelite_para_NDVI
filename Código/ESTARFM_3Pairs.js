...
fine3 - Landsat Image at date 3 (pair)
...
*/
var predict = function(window_pixels, k, band, roi, date_pair1, date_pair2, date_pair3, lst_predicted) {

    var modis_pair1 = ee.Image(modis
        .filterBounds(roi)
        .map(addTime)
        .filterMetadata('Date', 'equals', date_pair1.get('Date'))
        ...
        crs: date_pair1.select(band).projection()
    })

    var modis_pair2 = ee.Image(modis
        .filterBounds(roi)
        .map(addTime)
        .filterMetadata('Date', 'equals', date_pair2.get('Date'))
        .select(['sur_refl_b02', 'sur_refl_b01', 'Time', 'QC_250m'], ['nir', 'red', 'Time', 'QC_250m'])
        .map(addNDVI).map(maskCloudsModis).first()).resample('bilinear').reproject({
        crs: date_pair2.select(band).projection()
    })

    var modis_pair3 = ee.Image(modis
        .filterBounds(roi)
        .map(addTime)
        .filterMetadata('Date', 'equals', date_pair3.get('Date'))
        .select(['sur_refl_b02', 'sur_refl_b01', 'Time', 'QC_250m'], ['nir', 'red', 'Time', 'QC_250m'])
        .map(addNDVI).map(maskCloudsModis).first()).resample('bilinear').reproject({
        crs: date_pair3.select(band).projection()
    })
    ...
    var f1 = ee.ImageCollection(date_pair1).select([band, 'latitude', 'longitude', 'Time'])
        .map(function(img) {
    ...
    var f2 = ee.ImageCollection(date_pair2).select([band, 'latitude', 'longitude', 'Time'])
        .map(function(img) {
    ...
    var f3 = ee.ImageCollection(date_pair3).select([band, 'latitude', 'longitude', 'Time'])
        .map(function(img) {
            return img.addBands(modis_pair3.select(band))
                .addBands(modis_predicted.select(band))
                .rename([band + '_LSTPair', 'latitude', 'longitude', 'Time', band + '_ModisPair', band + '_ModisPredicted'])
                .set('Time', img.get('system:time_start'))
        })
    ...
    var coef3 = conversionCoeff(f3, modis_pair3, modis_predicted)
    ...
    var predict3 = predictByImage(img_fine3, weight3, img_fine3)

    var final1 = temporalWeight(img_fine1).divide(temporalWeight(img_fine1).add(temporalWeight(img_fine2)).add(temporalWeight(img_fine3))).multiply(predict1).rename(band)
    var final2 = temporalWeight(img_fine2).divide(temporalWeight(img_fine1).add(temporalWeight(img_fine2)).add(temporalWeight(img_fine3))).multiply(predict2).rename(band)
    var final3 = temporalWeight(img_fine3).divide(temporalWeight(img_fine1).add(temporalWeight(img_fine2)).add(temporalWeight(img_fine3))).multiply(predict3).rename(band)


    var predicted = final1.add(final2.add(final3)).addBands(img_fine1.select('nCandidates').add(img_fine2.select('nCandidates')).add(img_fine3.select('nCandidates')))
        .addBands(modis_predicted.select(band).rename(band + '_ModisPredicted')).addBands(ee.Image.pixelLonLat())
        .addBands(lst_predicted.select([band], [band + '_LSTPredicted']))


    return predicted.addBands(predicted.select(band + '_LSTPredicted').subtract(predicted.select(band)).rename('error_LST'))
...
var histograms = function(date_pair1, date_pair2, date_pair3, date_predicted, roi, roi_name, flag) {
    ...
    if (flag) {
        ...
        w.forEach(function(nameW, indexW) {
            k.forEach(function(nameK, indexK) {
                ...
                var pre = predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted)
            ...
            var k4 = predict(w[indexW], 4, 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).select(['NDVI'], ['NDVI_K4']).updateMask(date_predicted.select('NDVI'))
            var k6 = predict(w[indexW], 6, 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).select(['NDVI'], ['NDVI_K6']).updateMask(date_predicted.select('NDVI'))
            var k8 = predict(w[indexW], 8, 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).select(['NDVI'], ['NDVI_K8']).addBands(k4).addBands(k6).updateMask(date_predicted.select('NDVI').unmask(-2).neq(-2)).updateMask(k4.neq(0))
        ...
        var image = predict(3, 4, 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).updateMask(date_predicted.select('NDVI'))
        ...
        w.forEach(function(nameW, indexW) {
            k.forEach(function(nameK, indexK) {
                ...
                var imageNIR = predict(w[indexW], k[indexK], 'nir', roi, date_pair1, date_pair2, date_pair3, date_predicted);
                var imageRED = predict(w[indexW], k[indexK], 'red', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            ...
            // K=4
            var imageNIR4 = predict(w[indexW], 4, 'nir', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            var imageRED4 = predict(w[indexW], 4, 'red', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            ...
            // K=6
            var imageNIR6 = predict(w[indexW], 6, 'nir', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            var imageRED6 = predict(w[indexW], 6, 'red', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            ...
            // K=8
            var imageNIR8 = predict(w[indexW], 8, 'nir', roi, date_pair1, date_pair2, date_pair3, date_predicted);
            var imageRED8 = predict(w[indexW], 8, 'red', roi, date_pair1, date_pair2, date_pair3, date_predicted);
...
var scatters = function(date_pair1, date_pair2, date_pair3, date_predicted, roi, roi_name, flag) {
    ...
    if (flag) {
        w.forEach(function(nameW, indexW) {
            k.forEach(function(nameK, indexK) {
                ...
                var predicted = predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted)
...
app.createHelpers = function() {
    ...
    app.applyDates = function() {
        ...
        computedIds.evaluate(function(ids) {
            ...
            app.picker.pairDate3.select.items().reset(ids);
            ...
            app.picker.pairDate3.select.setValue(app.picker.pairDate3.select.items().get(0));
        });
    ...
    app.refreshMapCompare = function() {
        ...
        var pairDate3 = ee.Image(collection.filterMetadata('system:index', 'equals', app.picker.pairDate3.select.getValue()).first());
        ...
        if (pairDate1 && pairDate2 && pairDate3 && originalDate) {
            ...
            if (app.KW.applyNDVI.getValue()) {
                ...
                var predicted = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'NDVI', roi,
                    pairDate1, pairDate2, pairDate3, originalDate);
    ...
    app.refreshMapOriginalModis = function() {
        ...
        var map = ui.Map()
        ui.root.widgets().set(0, map);
        map.add(ui.Label('Original Modis Image'))
        map.add(legend)
        var roi = app.ROI_OPTIONS[app.roi.select.getValue()].roi
        map.centerObject(roi, 12);
        map.addLayer(roi, {}, 'ROI')
        ...
        if (pairDate1 && pairDate2 && predictedDate) {
            if (app.KW.applyNDVI.getValue()) {
                ...
                var image = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'NDVI', roi,
                    pairDate1, pairDate2, pairDate3, predictedDate);
                // Add the image to the map with the corresponding visualization options.
                //var visOption = app.VIS_OPTIONS[app.vis.select.getValue()];
    ...
    app.refreshMapPairs = function() {
        ...
        if (pairDate1 && pairDate2 && pairDate3) {
            // Pair 1
            ...
            // Pair2
            maps[2].addLayer(pairDate2.select('NDVI').clip(roi), ndviParams, 'Pair1LST');
            maps[2].addLayer(pairDate2.select('NDVI').unmask(-2).eq(-2).updateMask(pairDate2.select('NDVI').unmask(-2).eq(-2)).clip(roi), {
                palette: '#b30000'
            }, 'masked');
            print(pairDate2.reduceRegion(ee.Reducer.mean(), roi, 30), 'Mean Pair1 LST')
            ...
            // Pair3
            maps[4].addLayer(pairDate3.select('NDVI').clip(roi), ndviParams, 'Pair3LST');
            maps[4].addLayer(pairDate3.select('NDVI').unmask(-2).eq(-2).updateMask(pairDate3.select('NDVI').unmask(-2).eq(-2)).clip(roi), {
                palette: '#b30000'
            }, 'masked');
            print(pairDate3.reduceRegion(ee.Reducer.mean(), roi, 30), 'Mean Pair3 LST')

            var modisPair3 = ee.Image(modis
                .filterBounds(roi)
                .map(addTime)
                .filterMetadata('Date', 'equals', pairDate3.get('Date'))
                .select(['sur_refl_b02', 'sur_refl_b01', 'Time', 'QC_250m'], ['nir', 'red', 'Time', 'QC_250m'])
                .map(addNDVI).map(maskCloudsModis).first()).resample('bilinear').reproject({
                crs: pairDate3.select('nir').projection()
            })

            print(modisPair3.reduceRegion(ee.Reducer.mean(), roi, 30), 'Mean Pair3 Modis')
            maps[5].addLayer(modisPair3.select('NDVI').clip(roi), ndviParams, 'PairModis3');
        }
    ...
    app.refreshMapClusters = function() {
        ...
        if (app.KW.applyNDVI.getValue()) {
            if (pairDate1 && pairDate2 && pairDate3 && predictedDate) {
                ...
                maps[2].addLayer(clusters(pairDate3, 'NDVI', k, roi).select('cluster').clip(app.ROI_OPTIONS[app.roi.select.getValue()].roi).randomVisualizer(), {}, 'Clusters-PairImage3');
            }
        } else {
            print('Impossible to Show Clusters. Select apply NDVI at first')
        }
        ui.root.widgets().set(0, mapGrid);
    };
    ...
    app.showHistograms = function() {
        ...
        var date_pair3 = ee.Image(collection.filterMetadata('system:index', 'equals', app.picker.pairDate3.select.getValue()).first())


        print('--------- Histograms--------------')
        histograms(date_pair1, date_pair2, date_pair3, date_predicted, roi, roi_name, app.KW.applyNDVI.getValue());
    }
    ...
    app.showScatters = function() {
        ...
        print('--------- Scatter-----------------')
        scatters(date_pair1, date_pair2, date_pair3, date_predicted, roi, roi_name, app.KW.applyNDVI.getValue());
    ...
    app.metrics = function() {
        ...
        if (app.KW.applyNDVI.getValue()) {
            ...
            w.forEach(function(nameW, indexW) {
                k.forEach(function(nameK, indexK) {
                    print('---------K=' + nameK + '------------')
                    var image = predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted)
                    ...
                    print(predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).updateMask(image.select('NDVI').neq(0)).reduceRegion(ee.Reducer.mean(), roi, 30), 'Mean Predicted')


                    print(image.select(['NDVI', 'NDVI_LSTPredicted']).reduceRegion(ee.Reducer.pearsonsCorrelation(), roi, 30), 'pearsons Correlation');
                    print(predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted).updateMask(image.select('NDVI').neq(0)).reduceRegion(ee.Reducer.mean(), roi, 30), 'Mean Predicted Image W' + nameW + 'K' + nameK)
            ...
            w.forEach(function(nameW, indexW) {
                k.forEach(function(nameK, indexK) {
                    ...
                    print('---------K=' + nameK + '------------')
                    var imageNIR = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'nir', roi,
                        date_pair1, date_pair2, date_pair3, date_predicted);
                    var imageRED = predict(app.W_OPTIONS[app.KW.W.select.getValue()].w, app.K_OPTIONS[app.KW.K.select.getValue()].k, 'red', roi,
                        date_pair1, date_pair2, date_pair3, date_predicted);
                    var ndvi = imageNIR.select('nir').addBands(imageRED.select('red')).normalizedDifference(['nir', 'red']).rename('NDVI');
                    var error_LST = date_predicted.select('NDVI').subtract(ndvi.select('NDVI')).rename('error_LST')
    ...
    app.exportPredictedImage = function() {
        ...
        if (predictedDate && pairDate1 && pairDate2 && pairDate3) {
            if (app.KW.applyNDVI.getValue()) {
    ...
    app.exportPairs = function() {
        ...
        if (pairDate1 && pairDate2 && pairDate3) {
            ...
            Export.image.toDrive({
                image: pairDate3.select('NDVI').clip(roi).visualize(ndviParams),
                description: 'L8_Pair2-' + app.picker.pairDate3.select.getValue(),
                scale: 30,
                region: roi
            });
            ...
            var modisPair3 = ee.Image(modis
                ...
                .select(['sur_refl_b02', 'sur_refl_b01', 'Time', 'QC_250m'], ['nir', 'red', 'Time', 'QC_250m']).map(maskCloudsModis)
                .map(addNDVI).first()).resample('bilinear').reproject({
                crs: pairDate3.select('NDVI').projection()
            })

            Export.image.toDrive({
                image: modisPair3.select('NDVI').clip(roi).visualize(ndviParams),
                description: 'Modis_Pair3-' + app.picker.pairDate3.select.getValue(),
                scale: 30,
                region: roi
            });
        }
    ...
    app.exportMetrics = function() {
        ...
        var makePrediction = function(indexW, indexK, nameW, nameK, date_pair1, date_pair2, date_pair3, date_predicted) {
          var feats = ee.List([]);
          // NDVI
          var imageNDVI = predict(w[indexW], k[indexK], 'NDVI', roi, date_pair1, date_pair2, date_pair3, date_predicted)
          ...
          // Reflectance
          var imageNIR = predict(w[indexW], k[indexK], 'nir', roi, date_pair1, date_pair2, date_pair3, date_predicted);
          var imageRED = predict(w[indexW], k[indexK], 'red', roi, date_pair1, date_pair2, date_pair3, date_predicted);          var ndvi = imageNIR.select('nir').addBands(imageRED.select('red')).normalizedDifference(['nir', 'red']).rename('NDVI');
          var error_LST = date_predicted.select('NDVI').subtract(ndvi.select('NDVI')).rename('error_LST')
        ...
        w.forEach(function(nameW, indexW) {
            k.forEach(function(nameK, indexK) {
              ...
              // NO MASK
              var ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1, date_pair2, date_pair3, date_predicted)
              features = features.cat(ft)
              
              // WATER MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(watermask), date_pair2.updateMask(watermask), date_pair3.updateMask(watermask), date_predicted)
              featureswatermasked = featureswatermasked.cat(ft)
              
              // ROAD MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(roadmask), date_pair2.updateMask(roadmask), date_pair3.updateMask(roadmask), date_predicted)
              featuresroadsmasked = featuresroadsmasked.cat(ft)
              
              // BUILDING MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(buildingmask), date_pair2.updateMask(buildingmask), date_pair3.updateMask(buildingmask), date_predicted)
              featuresbuildingsmasked = featuresbuildingsmasked.cat(ft)
              
              // WR MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(watermask).updateMask(roadmask), date_pair2.updateMask(watermask).updateMask(roadmask), date_pair3.updateMask(watermask).updateMask(roadmask), date_predicted)
              featureswrmasked = featureswrmasked.cat(ft)
              
              // WB MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(watermask).updateMask(buildingmask), date_pair2.updateMask(watermask).updateMask(buildingmask), date_pair3.updateMask(watermask).updateMask(buildingmask), date_predicted)
              featureswbmasked = featureswbmasked.cat(ft)
              
              // RB MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(roadmask).updateMask(buildingmask), date_pair2.updateMask(roadmask).updateMask(buildingmask), date_pair3.updateMask(roadmask).updateMask(buildingmask), date_predicted)
              featuresrbmasked = featuresrbmasked.cat(ft)
              
              // ALL MASK
              ft = makePrediction(indexW, indexK, nameW, nameK, date_pair1.updateMask(watermask).updateMask(roadmask).updateMask(buildingmask), date_pair2.updateMask(watermask).updateMask(roadmask).updateMask(buildingmask), date_pair3.updateMask(watermask).updateMask(roadmask).updateMask(buildingmask), date_predicted)
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'ESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'WESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'RESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: 'BESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '1ESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '2ESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '3ESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
        ...
        Export.table.toDrive({
            collection: metrics,
            description: '4ESTARFM3_' + roi_abre + '_' + app.picker.pairDate1.select.getValue() + '_' +
                app.picker.pairDate2.select.getValue() + '_' + app.picker.pairDate3.select.getValue() + '_' + app.picker.predictedDate.select.getValue(),
            fileFormat: 'CSV',
            folder: 'metrics2',
            selectors: ['W', 'K', 'signal', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation']
...
app.createPanels = function() {
    ...
    app.intro = {
        panel: ui.Panel([
            ...
            ui.Label('This app allows you to apply ESTARFM using 3Pairs Landsat-Modis')
        ])
    };
    ...
    app.picker = {
        ...
        pairDate3: {
            ...
            select: ui.Select({
                placeholder: 'Select Image ID',
                onChange: function() {
                    var image = ee.Image(collection.filterMetadata('system:index', 'equals', app.picker.pairDate3.select.getValue()).first())
                    var cloud = image.get('CLOUD_COVER_REGION')
                    cloud.evaluate(function(result) {
                        app.picker.pairDate3.label.setValue('Cloud Cover in ROI: ' + result.toFixed(2))
                    })
                }
            })
        }
    ...
    app.picker.panel = ui.Panel({
        widgets: [
            ...
            ui.Panel([
                ui.Label('Pair Date3', app.HELPER_TEXT_STYLE), app.picker.pairDate3.select,
                app.picker.pairDate3.label
            ], ui.Panel.Layout.flow('horizontal')),
            ui.Panel([
