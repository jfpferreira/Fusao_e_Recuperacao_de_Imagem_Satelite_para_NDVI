...
var SantarClustered = ee.FeatureCollection("users/jfpferreira/KMeans/SantarKMeans");
var BeringelClustered = ee.FeatureCollection("users/jfpferreira/KMeans/BGLKMeans");
var SCTClustered = ee.FeatureCollection("users/jfpferreira/KMeans/SCTKMeans");
var BenficaClustered = ee.FeatureCollection("users/jfpferreira/KMeans/BenficaKMeans");
...
var linearRegression = function(image, band, roi, windowBefore, windowAfter) {
    ...
    var mean_pixels = predicted_lr.select(band)

    mean_pixels = mean_pixels.reduceRegion(ee.Reducer.mean(), roi, 30, null, null, true, 10000000, 4)

    mean_pixels = mean_pixels.toImage([band]).rename('mean_lr')
...
var fusion = function(window_pixels, prop, table, band, roi, collection, modis, lst_predicted) {
    var pixelGeometry;
    var resultList;
    ...
    var temp_clustering = function(img) {
        return img.addBands(resultList.select('tempcluster'))
    }
    ...
    var similarPixels = function(img) {
        ...
        var allCandidatesArrays = img.addBands(candidates('tempcluster', img))
        ...
        var mask_clusters = (allCandidatesArrays.select('tempcluster').eq(allCandidatesArrays.select('tempcluster_candidates'))).and(allCandidatesArrays.select('latitude')).and(allCandidatesArrays.select(band).neq(ee.Image(-2)))

        var image = allCandidatesArrays.select(band + '_candidates', band + '_ModisPair_candidates', band + '_ModisPredicted_candidates', 'tempcluster_candidates', 'slope_candidates', 'latitude_candidates', 'longitude_candidates').arrayMask(mask_clusters)

        var nCandidates = image.select('tempcluster_candidates').arrayLengths().arrayGet([0]).float().rename('nCandidates')
    ...
    pixelGeometry = colCoef.first().select('longitude','latitude').multiply(10000000).toInt64().reduceToVectors(
    { 
      'reducer': ee.Reducer.countDistinct(),
      'geometry': roi.geometry().buffer(100),
      'geometryType': 'polygon',
      'crs': colCoef.first().select('NDVI').projection(),
      'eightConnected': true, 
      'maxPixels': 700000000000  
    });
    pixelGeometry = pixelGeometry.toList(pixelGeometry.size())

    var sequence = ee.List.sequence(0, ee.Number(pixelGeometry.size()).subtract(1))
    resultList = ee.FeatureCollection(sequence.map(function(i){
        return ee.Feature(ee.Feature(pixelGeometry.get(i)).geometry())
                      .set(ee.Dictionary()
                      .set('cl', ee.List(table.aggregate_array(prop)).get(i)))
    })).reduceToImage(['cl'], ee.Reducer.first()).rename('tempcluster')
    
    var colClustering = colCoef.map(temp_clustering)
...
app.createHelpers = function() {
    ...
    app.refreshMapOriginalPredicted = function() {
        ...
        Map.clear();
        var roi = app.ROI_OPTIONS[app.roi.select.getValue()].roi
        var table = app.ROI_OPTIONS[app.roi.select.getValue()].table
        var k = app.K_OPTIONS[app.method.K.select.getValue()].k;
        var w = app.W_OPTIONS[app.filters.W.select.getValue()].w;
        var cc = app.CC_OPTIONS[app.method.CC.select.getValue()].cc;
        var imageToCorrect = ee.Image(collection.filterMetadata('system:index', 'equals', app.picker.imageToCorrect.select.getValue()).first())
        ...
        if (imageToCorrect) {
            ...
            var predicted = fusion(w, "K_"+ k + "_" + cc, table, 'NDVI', roi, collectionR, collectionModis, imageToCorrect).updateMask(imageToCorrect.select('NDVI').clip(roi).unmask(-2).neq(-2))
    ...
    app.refreshMapPredicted = function() {
        ...
        map.addLayer(ee.Image(predicted).select('NDVI').clip(roi), ndviParams, 'LCBFM');
    };
    ...
    app.exportTables = function() {
        ...
        var table = app.ROI_OPTIONS[app.roi.select.getValue()].table
        var clusters = table.first().propertyNames().removeAll(['system:index','Pixel', 'field_1'])
        ...
        var calc_Metrics_adaptive = function(date_predicted, image, windowBefore, windowAfter, windowRange, nameW, nameCluster, nameK, nameKTemp, nameCC, col, k_method, location) {
            ...
            var kTemp_dict = ee.Dictionary(['KTemp', nameKTemp])
            var k_m = ee.Dictionary(['Cluster', nameCluster])
            var cc_dict = ee.Dictionary(['CC', nameCC])
            ...
            var location = ee.Dictionary(['Local', location])
            var k_method = ee.Dictionary(['k_method', k_method])
            var dict = w_dict.combine(k_dict).combine(kTemp_dict).combine(k_m).combine(cc_dict).combine(r_dict).combine(mae).combine(rmse).combine(error_dict).combine(corr).combine(location).combine(k_method)
        ...
        var metrics = ee.List([]);
            // Adaptive - NDVI
            var col = collectionRecovered(imageToCorrect, 'NDVI', 4, roi, windowRange, 1, true, windowBefore, windowAfter);
            var values = ee.List([])
            metrics = metrics.cat(ee.List(clusters.iterate(function(cur,all){
                var feats = ee.List([])
                var nameV =  ee.String(cur)
                w.forEach(function(nameW, indexW) {
                    var date_predicted = imageToCorrect.select('NDVI').clip(roi)
                    var imageNDVI = fusion(w[indexW], nameV, table, 'NDVI', roi, col, collectionModis, imageToCorrect).updateMask(imageToCorrect.select('NDVI').clip(roi).unmask(-2).neq(-2))
                    var image = imageNDVI.select('NDVI').clip(roi)
                    var ft = calc_Metrics_adaptive(date_predicted, image, windowBefore, windowAfter, windowRange, nameW, nameV.slice(0,1), '', nameV.slice(2,3), nameV.slice(4), col, '4', roi_name)
                    feats = feats.cat(ee.List([ft]))
                })
                return ee.List(all).cat(feats)
            },values)))
        
        Export.table.toDrive({
            collection: ee.FeatureCollection(metrics),
            description: '4LCBFM_' + roi_abre + '_ADAPT_NDVI_B' + windowBefore + 'A' + windowAfter + '_R' + windowRange + '_' + date,
            fileFormat: 'CSV',
            folder: 'LCBFM',
            selectors: ['W', 'K', 'KTemp', 'Cluster', 'CC', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation', 'Local', 'k_method']
        })
        
        var metrics = ee.List([]);
        // Adaptive - NDVI
        var col = collectionRecovered(imageToCorrect, 'NDVI', 6, roi, windowRange, 1, true, windowBefore, windowAfter);
        var values = ee.List([])
        metrics = metrics.cat(ee.List(clusters.iterate(function(cur,all){
            var feats = ee.List([])
            var nameV =  ee.String(cur)
            w.forEach(function(nameW, indexW) {
                ...
                var imageNDVI = fusion(w[indexW], nameV, table, 'NDVI', roi, col, collectionModis, imageToCorrect).updateMask(imageToCorrect.select('NDVI').clip(roi).unmask(-2).neq(-2))
                var image = imageNDVI.select('NDVI').clip(roi)
                var ft = calc_Metrics_adaptive(date_predicted, image, windowBefore, windowAfter, windowRange, nameW, nameV.slice(0,1), '', nameV.slice(2,3), nameV.slice(4), col, '6', roi_name)
                feats = feats.cat(ee.List([ft]))
            })
            return ee.List(all).cat(feats)
        },values)))
        ...
        Export.table.toDrive({
            collection: ee.FeatureCollection(metrics),
            description: '6LCBFM_' + roi_abre + '_ADAPT_NDVI_B' + windowBefore + 'A' + windowAfter + '_R' + windowRange + '_' + date,
            fileFormat: 'CSV',
            folder: 'LCBFM',
            selectors: ['W', 'K', 'KTemp', 'Cluster', 'CC', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation', 'Local', 'k_method']
        })
        ...
        var metrics = ee.List([]);
        // Adaptive - NDVI
        var col = collectionRecovered(imageToCorrect, 'NDVI', 8, roi, windowRange, 1, true, windowBefore, windowAfter);
        var values = ee.List([])
        metrics = metrics.cat(ee.List(clusters.iterate(function(cur,all){
            var feats = ee.List([])
            var nameV =  ee.String(cur)
            w.forEach(function(nameW, indexW) {
                ...
                var imageNDVI = fusion(w[indexW], nameV, table, 'NDVI', roi, col, collectionModis, imageToCorrect).updateMask(imageToCorrect.select('NDVI').clip(roi).unmask(-2).neq(-2))
                var image = imageNDVI.select('NDVI').clip(roi)
                var ft = calc_Metrics_adaptive(date_predicted, image, windowBefore, windowAfter, windowRange, nameW, nameV.slice(0,1), '', nameV.slice(2,3), nameV.slice(4), col, '8', roi_name)
                feats = feats.cat(ee.List([ft]))
            })
            return ee.List(all).cat(feats)
        },values)))
        ...
        Export.table.toDrive({
            collection: ee.FeatureCollection(metrics),
            description: '8LCBFM_' + roi_abre + '_ADAPT_NDVI_B' + windowBefore + 'A' + windowAfter + '_R' + windowRange + '_' + date,
            fileFormat: 'CSV',
            folder: 'LCBFM',
            selectors: ['W', 'K', 'KTemp', 'Cluster', 'CC', 'R^2', 'RMSE', 'MAE', 'Error', 'correlation', 'Local', 'k_method']
        })
...
app.createPanels = function() {
    ...
    app.intro = {
        panel: ui.Panel([
            ui.Label({
                value: 'LCBFM (proposal) - Landsat/Modis',
                style: {
                    fontWeight: 'bold',
                    fontSize: '24px',
                    margin: '10px 5px',
    ...
    app.method = {
        ...
        CC: {
            ...
            // Create a select with a function that reacts to the "change" event.
            select: ui.Select({
                items: Object.keys(app.CC_OPTIONS),
                onChange: function() {
                    // Update the label's value with the select's description.
                    var option = app.CC_OPTIONS[app.method.CC.select.getValue()];
                    app.method.CC.label.setValue(option.description);
                }
            })
        }
    }
    ...
    app.method.panel = ui.Panel({
        widgets: [
            ...
            ui.Panel([
                ...
                ui.Label('CC', app.HELPER_TEXT_STYLE), app.method.CC.select,
    ...
    app.method.CC.select.setValue(app.method.CC.select.items().get(0));
...
app.createConstants = function() {
    ...
    app.ROI_OPTIONS = {
        'São Cristóvão de Lafões': {
            ...
            abre: 'SCTSCL',
            roi: saocristovao,
            table: SCTClustered.sort('Pixel', true)
        },
        'Benfica do Ribatejo ': {
            nameR: 'Benfira do Ribatejo',
            abre: 'BenRib',
            roi: benfica,
            table: BenficaClustered.sort('Pixel', true)
        },
        'Santar': {
            nameR: 'Santar',
            abre: 'Santar',
            roi: santar,
            table: SantarClustered.sort('Pixel', true)
        },
        'Beringel': {
            nameR: 'Beringel',
            abre: 'BGL',
            roi: beringel,
            table: BeringelClustered.sort('Pixel', true)
        }
    };
